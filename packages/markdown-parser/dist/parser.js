...
export class MarkdownParser {
        ...
        if (h4Matches.length > 0) {
            ...
            if (parts.code || parts.footer || cleanedContent.trim()) {
                ...
                const card = {
                    ...
                    isShortcutsCard: isShortcutsSection,
                    shortcuts: isShortcutsSection
                        ? this.parseShortcutsFromContent(parts.footer || parts.code || cleanedContent)
                        : undefined,
                };
                cards.push(card);
            }
     ...
     */
    async parseIndividual(filePaths, options = {}, outputDir = "./public/data/cheatsheets", indexOutputPath = "./public/data/cheatsheets-index.json") {
        const indexItems = [];
        for (const filePath of filePaths) {
            try {
                const result = await this.parseFile(filePath, options);
                if (result.success && result.document) {
                    ...
                    const individualOutputPath = path.join(outputDir, `${cheatsheetId}.json`);
                    const cheatsheetData = {
                        id: cheatsheetId,
                        metadata: result.document.metadata,
                        sections: result.document.sections,
                    };
                    await fs.promises.writeFile(individualOutputPath, JSON.stringify(cheatsheetData, null, 2), "utf-8");
                    // Create index item
                    const indexItem = {
                        id: cheatsheetId,
                        name: result.document.metadata.title || fileName,
                        description: result.document.metadata.intro ||
                            `Quick reference for ${result.document.metadata.title || fileName}`,
        ...
        await fs.promises.writeFile(indexOutputPath, JSON.stringify(indexData, null, 2), "utf-8");
        return indexData;
    }
    /**
     * Generate keywords from cheatsheet content
     */
    generateKeywords(document) {
     ...
     */
    getIcon(categories) {
        if (categories.includes("Keyboard Shortcuts"))
            return "Code";
        if (categories.includes("Programming"))
            return "Code";
        return "BookOpen";
    }
}
