...
program
    ...
    .option("-o, --output <path>", "Output file path (defaults to <filename>.json)")
    ...
    try {
        ...
        const jsonOutput = parser.toJSON(result.document, options.pretty ? 2 : 0);
        await fs.promises.writeFile(outputPath, jsonOutput, "utf-8");
        console.log(chalk.green(`✅ Successfully parsed and saved to: ${outputPath}`));
        // Print summary
        const doc = result.document;
        console.log(chalk.cyan("\n📊 Summary:"));
        console.log(`   Title: ${doc.metadata.title || "N/A"}`);
        console.log(`   Sections: ${doc.sections.length}`);
        console.log(`   Total Cards: ${doc.sections.reduce((total, section) => total +
            section.subsections.reduce((subTotal, sub) => subTotal + sub.cards.length, 0), 0)}`);
    }
    catch (error) {
...
program
    ...
    .option("-o, --output-dir <path>", "Output directory (defaults to same directory as input files)")
    .option("--no-metadata", "Skip metadata extraction")
    .option("--no-code-blocks", "Do not preserve code block formatting")
    .option("--no-span-config", "Do not extract span configuration")
    .option("--pretty", "Pretty print JSON output")
    ...
    try {
        ...
        for (const result of results) {
            if (result.success) {
                ...
                const outputPath = path.join(outputDir, `${path.parse(inputFile).name}.json`);
                const jsonOutput = parser.toJSON(result.document, options.pretty ? 2 : 0);
                await fs.promises.writeFile(outputPath, jsonOutput, "utf-8");
                console.log(chalk.green(`✅ ${path.basename(inputFile)} → ${path.basename(outputPath)}`));
                successCount++;
            }
            else {
                console.error(chalk.red(`❌ ${path.basename(result.filePath)}: ${result.error}`));
                errorCount++;
            }
        }
        console.log(chalk.cyan(`\n📊 Batch Summary:`));
...
program
    ...
    if (!fs.existsSync(exampleFile)) {
        ...
        console.log(chalk.yellow("Make sure the python.md example file exists in the examples directory."));
        process.exit(1);
    }
    try {
        const parser = new MarkdownParser();
        ...
        const outputPath = path.join(__dirname, "../examples/python-cheatsheet-parsed.json");
        const jsonOutput = parser.toJSON(result.document, options.pretty ? 2 : 0);
        await fs.promises.writeFile(outputPath, jsonOutput, "utf-8");
