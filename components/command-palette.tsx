...
import { ArrowRight, Command, Search, Code, BookOpen } from "lucide-react";
...
export function CommandPalette() {
  ...
  useEffect(() => {
    loadCheatsheetsIndex()
      .then((data) => {
        const cheatsheetsForPalette = data.cheatsheets.map((cheatsheet) => ({
          name: cheatsheet.name,
          description: cheatsheet.description,
          href: `/cheatsheets/${cheatsheet.id}`,
          icon: cheatsheet.icon === "BookOpen" ? BookOpen : Code,
          category: "Reference",
          keywords: [
            ...cheatsheet.keywords,
        ...
        setCheatsheets(cheatsheetsForPalette);
      })
      .catch((error) => {
        console.error("Failed to load cheatsheets for command palette:", error);
      });
  }, []);
  ...
  const filteredItems = useMemo(() => {
    ...
    return [
      ...
      ...searchTools(query),
      ...cheatsheets.filter((cheatsheet) => {
        const searchText = query.toLowerCase();
        return (
          cheatsheet.name.toLowerCase().includes(searchText) ||
          cheatsheet.description.toLowerCase().includes(searchText) ||
          cheatsheet.keywords.some((keyword: string) =>
            keyword.toLowerCase().includes(searchText)
          )
        );
      }),
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-2xl p-0 gap-0 bg-white dark:bg-slate-900 border-gray-200 dark:border-gray-700">
        ...
        <div className="flex items-center border-b border-gray-200 dark:border-gray-700 px-4 py-3">
          ...
          <Input
            ref={inputRef}
            placeholder="Search tools, cheatsheets and actions..."
            value={query}
          ...
          <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-1">
                <kbd className="px-1.5 py-0.5 bg-white dark:bg-slate-700 border border-gray-300 dark:border-gray-600 rounded text-xs">
                  ↑↓
                </kbd>
                <span>Navigate</span>
              </div>
